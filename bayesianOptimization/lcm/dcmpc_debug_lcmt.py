...
class dcmpc_debug_lcmt(object):
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(dcmpc_debug_lcmt._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != dcmpc_debug_lcmt._get_packed_fingerprint():
            raise ValueError("Decode error")
        return dcmpc_debug_lcmt._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = dcmpc_debug_lcmt()
    ...
    def _get_hash_recursive(parents):
        if dcmpc_debug_lcmt in parents: return 0
    ...
    def _get_packed_fingerprint():
        if dcmpc_debug_lcmt._packed_fingerprint is None:
            dcmpc_debug_lcmt._packed_fingerprint = struct.pack(">Q", dcmpc_debug_lcmt._get_hash_recursive([]))
        return dcmpc_debug_lcmt._packed_fingerprint
